 ...
 * under the License.
 */
package org.apache.iceberg.geometry.serde;
...
@SuppressWarnings({"BadAssert", "checkstyle:LocalVariableName"})
class UnsafeGeometryBuffer implements GeometryBuffer {
  ...
  UnsafeGeometryBuffer(int bufferSize) {
    bytes = new byte[bufferSize];
    baseOffset = BYTE_ARRAY_BASE_OFFSET;
  }

  UnsafeGeometryBuffer(byte[] bytes, int offset) {
    this.bytes = bytes;
    baseOffset = offset + BYTE_ARRAY_BASE_OFFSET;
  }

  UnsafeGeometryBuffer(byte[] bytes) {
    this.bytes = bytes;
    baseOffset = BYTE_ARRAY_BASE_OFFSET;
  }
